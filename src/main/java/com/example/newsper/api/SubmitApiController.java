...
public class SubmitApiController {
    ...
    public ResponseEntity<?> view(
        ...
        if(user.getRole().equals("associate") && !submitEntity.getUserId().equals(userId)) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(errorCodeService.setErrorCodeBody(-301));
    ...
    public ResponseEntity<?> create(
        ...
        if(!user.getRole().equals("associate")) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(errorCodeService.setErrorCodeBody(-302));
    ...
    public ResponseEntity<?> update(
        ...
        if(!submitEntity.getUserId().equals(userId)) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(errorCodeService.setErrorCodeBody(-303));
    ...
    public ResponseEntity<?> delete(
        ...
        //!(관리자 || 과제 작성자 || 과제 제출자)
        if(!(user.getRole().equals("admin") || assignmentEntity.getUserId().equals(userId) || submitEntity.getUserId().equals(userId))) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(errorCodeService.setErrorCodeBody(-303));
